...
function BookingscreenM() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setloading(true);
        const data = (
          await axios.post("/api/taxis/gettaxibyid", { taxiid: id })
        ).data;
  ...
  useEffect(() => {
    if (taxi) {
      let amount = totalDays * taxi.rentperday;
  ...
  async function onToken(token) {
    ...
    try {
      setloading(true);
      const result = await axios.post("/api/bookings/booktaxi", bookingDetails);

      setloading(false);
      Swal.fire(
        "Congratulations !",
        "Your Taxi is reserved Successfully",
        "success"
      ).then(result => {
        ...
        <div>
          <div className="row justify-content-center mt-5 bs">
            <div className="col-md-5">
              <h1>{taxi.name}</h1>
              <img src={taxi.imageurls[0]} className="bigimg" />
            </div>
            <div className="col-md-5 ">
